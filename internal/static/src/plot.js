...
const heatmapBucketLabel = (bucket, edges, formatYUnit, minimum) => {
  if (bucket === 0) {
    if (minimum != null) {
      return `[${formatYUnit(minimum)}, ${formatYUnit(edges[bucket])})`;
    }
    return `(-Inf, ${formatYUnit(edges[bucket])})`;
  }
  if (bucket === edges.length - 1) {
    return `[${formatYUnit(edges[bucket - 1])}, +Inf)`;
  }
  ...
  constructor(cfg) {
    ...
    if (this.#cfg.type == "heatmap") {
      ...
      this.#bucketLabels = edges.map((_, y) =>
        heatmapBucketLabel(y, edges, formatYUnit, hover.ymin),
      );
